...
mmerge appln_id using "$patstpath/general/appln_info", unmatched(none) ukeep(appln_auth appln_year earliest_filing_year earliest_publn_year ipr_type granted)
keep if ipr_type==2
drop ipr_type
drop _m
mmerge appln_id using "$patstpath/general/family_info", unmatched(master) ukeep(docdb_family_id)
drop _m
mmerge docdb_family_id using "$patstpath/general/docdb_families", unmatched(master) ukeep(famsize_patents famsize_off*)
ren famsize_patents famsize_patents_docdb
ren famsize_offices famsize_offices_docdb
ren famsize_offices_EPCgrants famsize_offices_EPCgrants_docdb
drop _m
mmerge appln_id using "$patstpath/general/appln_applicant_country_nomissing", unmatched(master) ukeep(applt)
drop _m
mmerge appln_id using "$patstpath/general/appln_inventor_country_nomissing", unmatched(master) ukeep(invt)
drop _m
mmerge appln_id using "$patstpath/general/appln_BvDID", unmatched(master) ukeep(BvDIDnumber)
drop _m
ren BvDIDnumber invt_firm
...
*duplicates drop docdb_family_id technology invt_country appln_auth, force
...
*NOTE: currently, we use the following data sets for the analysis:
*all_CCMTation_patents_basic_information.dta, inventions_tech_inventor_ctry_year.dta, transfers_tech_inventor_ctry_office_year.dta


use "$patstpath/mitigation/all_mitigation_patents_info", clear
*keep if technology==1
keep appln_id
duplicates drop
...
sort technology invt_country publn_year
mmerge invt_country publn_year technology  using "$patstpath/mitigation/Complete_4digits_ALL_inventions_by_invtcountry_year"
drop _m
...
drop _m
save "$datapath/databases/CCMT_4digits_tech_invt_ctry_year_Induced", replace
...
mmerge invt_iso technology publn_year using "$datapath/databases/CCMT_4digits_tech_invt_ctry_year_Induced", unmatched(master)
...
keep if publn_year>1969 & publn_year<2018
mmerge technology appln_auth publn_year invt_country using "$patstpath/mitigation/Complete_4digits_ALL_patents_invt_auth_year.dta"
drop _m
ren invt_country iso2
mmerge iso2 using "$otherpath/CountryID", unmatched(none) ukeep(iso3)
ren iso3 invt_iso
...
mmerge invt_iso technology patent_office_iso publn_year using "$datapath/databases/CCMT_4digits_tech_invt_auth_publn_year_Induced", ///
unmatched(master)
